...
- `int` — parse a string as an integer, truncating decimals.
...
- `date_add(n, unit)` — add (or subtract with negative `n`) time to a date. Units: `seconds`, `minutes`, `hours`, `days`, `weeks`, `months`, `years`.
- `date_diff(date2, unit)` — difference between two dates in the given unit (`date2 − receiver`).
